...
## Changelog — critical fixes in this revision

1. **Navigation simplified**: CPU Lab, LockGuard, Processes, Memory, and
   I/O are no longer separate top-level tabs — they're now sub-tabs
   *inside* Experiment Lab (pill buttons at the top of that page). Nav is
   now just: Dashboard, Experiment Lab, Analytics, Execution Trace, Learn.
2. **Memory experiment crash fixed at the root cause.** The old version
   ...
   I proved this by sending SIGKILL to the child process directly and
   confirming the parent (and `memory_experiment.status()`) stayed alive.
3. **Added the missing result graphs**: Memory (before/during bar chart),
   Starvation (per-thread avg-wait bar chart), I/O (write/read time bar
   chart), and Deadlock Guard ON/OFF comparison (detection-time bar
   chart) — using Chart.js, same as the CPU Lab's live chart already had.
