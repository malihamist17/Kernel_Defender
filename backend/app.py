...
def get_incidents():
    combined = list(reversed(INCIDENTS[-50:]))
    return combined if combined else db.get_incidents(50)
...
def run_memory_experiment(target_mb: int = 512, hold_seconds: int = 5):
...
def run_tiered(mode: str = "baseline"):
    """Run the tiered memory policy engine. mode = baseline | adaptive."""
    if mode not in ("baseline", "adaptive"):
        return {"error": "mode must be 'baseline' or 'adaptive'"}

    tracelog.log("COMMAND", "Tiered Lab", f"./tiered {mode}")

    try:
        proc = subprocess.run(
            ["/home/mahi-rat/tiered", mode],
            capture_output=True, text=True, timeout=30,
        )
    except FileNotFoundError:
        return {"error": "binary not found at /home/mahi-rat/tiered"}
    ...
    for m in re.finditer(
        r"\[(after [^\]]+)\]\s+VmRSS=(\d+)\s+kB\s+VmSwap=(\d+)\s+kB", output
    ):
        stages[m.group(1)] = {
            "vmrss_kb": int(m.group(2)),
            "vmswap_kb": int(m.group(3)),
        }

    demoted = None
    dm = re.search(r"queued (\d+) pages for demotion", output)
    if dm:
        demoted = int(dm.group(1))
    ...
    new_incident("tiered_memory", "LOW",
                 f"Tiered policy '{mode}': VmSwap after = "
                 f"{stages.get('after policy', {}).get('vmswap_kb', '?')} kB")
    db.save_experiment("tiered", result)
    tracelog.log("RESULT", "Tiered Lab", f"{mode} complete", result)
    return result
...
def run_tiered_visual(mode: str = "adaptive"):
    ...
    try:
        proc = subprocess.run(
            ["/home/mahi-rat/tiered_live", mode, "--json"],
            capture_output=True, text=True, timeout=90,
        )
    except FileNotFoundError:
        return {"error": "binary not found at /home/mahi-rat/tiered_live"}
    except subprocess.TimeoutExpired:
        return {"error": "timed out"}
    out = proc.stdout
    start = out.find("{")
    ...
    if start < 0 or end <= start:
        return {"error": "no JSON in output", "raw": out[-400:]}
    import json as jsonlib
    try:
        data = jsonlib.loads(out[start:end])
    except Exception as e:
        return {"error": f"json parse failed: {e}", "raw": out[-400:]}
    data["mode"] = mode
    tracelog.log("RESULT", "Tiered Lab", f"visual {mode}", {"resident": data.get("resident")})
    return data
@app.post("/api/experiments/paging/analyze")
def paging_analyze():
    try:
        proc = subprocess.run(
            ["/home/mahi-rat/paging_analysis", "--json"],
            capture_output=True, text=True, timeout=90,
        )
    except FileNotFoundError:
        return {"error": "binary not found at /home/mahi-rat/paging_analysis"}
    except subprocess.TimeoutExpired:
        return {"error": "timed out"}
    out = proc.stdout
    start = out.find("{")
    ...
    try:
        ...
        return {"error": f"json parse failed: {e}", "raw": out[-400:]}
    tracelog.log("RESULT", "Paging Analysis",
                 f"present={data.get('present')} swapped={data.get('swapped')}")
    return data
@app.post("/api/experiments/phase_shift/run")
def run_phase_shift():
    try:
        proc = subprocess.run(
            ["/home/mahi-rat/phase_shift", "--json"],
            capture_output=True, text=True, timeout=180,
        )
    except FileNotFoundError:
        return {"error": "binary not found at /home/mahi-rat/phase_shift"}
    except subprocess.TimeoutExpired:
        return {"error": "timed out"}
    out = proc.stdout
    start = out.find("{")
    ...
    try:
        ...
        return {"error": f"json parse failed: {e}", "raw": out[-400:]}
    tracelog.log("RESULT", "Phase Shift",
                 f"naive_B={data.get('naive_b')} adapt_B={data.get('adapt_b')}")
    return data
#---------------------
# ---------------- LockGuard: Deadlock ----------------
...
LEARN_CONTENT = {
    ...
    "body": "Modern servers increasingly have multiple tiers of memory — "
            "fast local DRAM and slower CXL-attached memory. The OS must "
            "decide, continuously, which pages belong in the fast tier. "
            "Our Tiered Lab builds a real policy engine using MADV_PAGEOUT "
            "(demote) and MADV_WILLNEED (promote) syscalls, measured via "
            "/proc/self/status VmRSS/VmSwap. Baseline policy keeps "
            "everything in RAM; adaptive policy demotes cold pages to "
            "swap, keeping only the hot set resident.",
},
    "paging_analysis": {
    "title": "Virtual-to-Physical Address Translation & Multi-Level Paging",
    "body": "On x86_64, a 48-bit virtual address is split into five fields: "
            "a 9-bit PGD index (bits 47:39), a 9-bit PUD index (bits 38:30), "
            "a 9-bit PMD index (bits 29:21), a 9-bit PTE index (bits 20:12), "
            "and a 12-bit page offset. The CPU walks four page tables "
            "(PGD->PUD->PMD->PTE) to translate a virtual address to a physical "
            "frame. Our Paging Analysis reads /proc/self/pagemap, which gives "
            "one 64-bit entry per 4 KB page: bit 63 is the present bit (page "
            "in RAM), bit 62 is the swapped bit (page in swap). By demoting "
            "cold pages with madvise(MADV_PAGEOUT), we observe the kernel "
            "clear the present bit and set the swapped bit per page, then "
            "restore them on access via a real page fault. This is the "
            "per-page ground truth of the tiering effect.",
},
        "phase_shift": {
        "title": "Phase-Shifting Workloads and the Limit of Userspace Policy",
        "body": "We tested whether our userspace policy can beat the kernel's "
                "default LRU when a workload's hot set changes mid-run. "
                "Result: both policies (frequency-only and recency-aware) "
                "produce identical outcomes — the kernel's own LRU already "
                "handles phase shifts of this class. This is a negative "
                "result, and it maps the boundary: MADV_PAGEOUT is a hint, "
                "not a control channel. The research direction is not to "
                "replace the kernel's LRU, but to extend it for hardware "
                "(like CXL) whose latency profile the LRU does not model.",
    },
}
